         ...
         {loading ? (
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
             {friendsData.map((friend) => {
               // Find segments for stats
               const segments = friend.data?.segments || [];
               
               // Get Battle Royale stats for Win %
               const battleRoyaleSegment = segments.find((s: any) => {
                 const type = s.type?.toLowerCase();
                 const name = s.metadata?.name?.toLowerCase() || '';
                 return type === 'battle-royale' || type === 'br' || name.includes('battle royale');
               ...
               return (
                 <FriendCard
                   ...
                   kd={overallStats?.kdRatio?.displayValue} // Use overall K/D ratio
                   wl={brStats?.wlPercentage?.displayValue || brStats?.wins?.displayValue} // Use BR Win % or Wins
